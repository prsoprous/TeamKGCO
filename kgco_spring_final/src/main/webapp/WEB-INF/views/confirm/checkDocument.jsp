...
<body>
	<div id="divDocumentGrid">
		...
		<div id="divDocumentHeadGrid">
			...
			<div id="divDocumentconfirm">
				<table>
					...
					<tr>
						<!-- 승인자 -->
						<td>${documentVO.confirm1}</td>
						<td>${documentVO.confirm2}</td>
						<td>${documentVO.confirm3}</td>
					</tr>
				</table>
			</div>
		</div>
		...
		<!-- 결제, 취소, 닫기 만들기 -->
<div id="btnBox">
      <sec:authentication property="principal.employee.eid" var="userid"/>
      <button  onclick="location.href='/confirm/confirm?no=${documentVO.documentNo}&check=confrim&user=${userid}'">결제</button>
      <!-- 보고 -->&nbsp;&nbsp;
      <button onclick="location.href='/confirm/confirm?no=${documentVO.documentNo}&check=cancel&user=${userid}'">반려</button>
      &nbsp;&nbsp;
